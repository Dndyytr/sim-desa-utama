...
import { index } from '@/routes/village-agendas';
...
export default function VillageAgendaEdit({
    agenda,
}: {
    ...
    const [posterPreview, setPosterPreview] = useState<string | null>(
        agenda.poster ? `/storage/${agenda.poster}` : null,
    );
    const [attachmentName, setAttachmentName] = useState<string | null>(
        agenda.attachment
            ? agenda.attachment.split('/').pop() || 'File Lampiran'
            : null,
    );
    ...
    const { data, setData, post, processing, errors } = useForm<{
        ...
        end_time: agenda.end_time.substring(0, 5),
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(
            route('village-agendas.update', {
                village_agenda: agenda.id,
            }),
            {
                forceFormData: true,
            },
        ...
        <>
            ...
            <div className="flex h-full flex-col gap-2 px-2 py-2 bp360:px-2.25 bp400:px-2.5 md:px-3 md:py-2.25 lg:px-3.5 lg:py-2.5 xl:px-4 xl:py-3 2xl:px-4.5 2xl:py-3.5">
                <span className="t-size2 font-medium text-stone-500">
                    Kelola Agenda Desa &gt; Ubah Agenda
                </span>
                        ...
                        <div className="inline-flex items-center gap-2 md:gap-2.5 lg:gap-2.75 xl:gap-3">
                            ...
                            <div>
                                <h2 className="t-size3 font-semibold text-(--primary)">
                                    Ubah Agenda / Kegiatan Desa
                                </h2>
                                <p className="t-size1 font-medium text-stone-500">
                                    Perbarui data agenda resmi Pemerintah Desa
                                    Utama berikut.
                                </p>
                            </div>
                        </div>
