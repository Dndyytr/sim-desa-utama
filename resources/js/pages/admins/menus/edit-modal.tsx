...
interface EditModalMenuProps {
    menus: Menu[];
    menu: Menu;
}

export default function EditModalMenu({ menus, menu }: EditModalMenuProps) {
    const [isOpen, setIsOpen] = useState(false);
    const initialParentId = menu.parent_id ? menu.parent_id.toString() : '';
    const initialIcon = menu.icon || '';
    const initialStatus = menu.status || '';

    const [selectedParentId, setSelectedParentId] =
        useState<string>(initialParentId);
    const [selectedIcon, setSelectedIcon] = useState<string>(initialIcon);
    const [selectedStatus, setSelectedStatus] = useState<string>(initialStatus);

    type UpdateMenuArg = Parameters<typeof update.form.patch>[0];

    const updateMenuArg = {
        id: menu.id,
    } as unknown as UpdateMenuArg;
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent>
                ...
                <DialogHeader className="t-size2">
                    ...
                    <DialogDescription>
                        Perbarui formulir untuk mengubah menu.
                    </DialogDescription>
                </DialogHeader>

                {/* <form onSubmit={handleSubmit} className="space-y-4"> */}
                <Form
                    {...update.form.patch(updateMenuArg)}
                    transform={(data) => ({
                        ...data,
                        parent_id:
